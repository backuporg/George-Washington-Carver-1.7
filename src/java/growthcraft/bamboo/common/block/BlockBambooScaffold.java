...
public class BlockBambooScaffold extends GrcBlockBase {
    ...
    private boolean checkSides(World world, BlockPos pos) {
        ...
        if (flag && world.getBlockState(x + 1, y - 1, z).isSideSolid(world, x + 1, y - 1, z, EnumFacing.UP))
            return true;
        if (flag1 && world.getBlockState(x - 1, y - 1, z).isSideSolid(world, x - 1, y - 1, z, EnumFacing.UP))
            return true;
        if (flag2 && world.getBlockState(x, y - 1, z + 1).isSideSolid(world, x, y - 1, z + 1, EnumFacing.UP))
            return true;
        if (flag3 && world.getBlockState(x, y - 1, z - 1).isSideSolid(world, x, y - 1, z - 1, EnumFacing.UP))
            return true;

        if (flag && world.getBlockState(x + 2, y - 1, z).isSideSolid(world, x + 2, y - 1, z, EnumFacing.UP))
            return true;
        if (flag1 && world.getBlockState(x - 2, y - 1, z).isSideSolid(world, x - 2, y - 1, z, EnumFacing.UP))
            return true;
        if (flag2 && world.getBlockState(x, y - 1, z + 2).isSideSolid(world, x, y - 1, z + 2, EnumFacing.UP))
            return true;
        return flag3 && world.getBlockState(x, y - 1, z - 2).isSideSolid(world, x, y - 1, z - 2, EnumFacing.UP);
